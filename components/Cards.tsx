      ...
      <div className="flex gap-3 sm:gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1.5 flex-wrap min-w-0">
            <span className="text-[9px] sm:text-[10px] font-bold px-2 py-0.5 bg-blue-50 text-blue-600 rounded-lg uppercase tracking-wider truncate max-w-[100px]">{question.course.split(':')[0]}</span>
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              <img src={question.userAvatar} className="w-5 h-5 sm:w-6 sm:h-6 rounded-lg shadow-sm shrink-0" alt="" />
              <div className="flex flex-col sm:flex-row sm:items-center sm:gap-2 min-w-0">
                <span className="text-[10px] sm:text-xs font-bold text-slate-700 truncate">{question.userName}</span>
                <span className="text-[10px] text-slate-400 hidden sm:inline">•</span>
                <span className="text-[10px] text-slate-400 shrink-0">{new Date(question.createdAt).toLocaleDateString()}</span>
              </div>
            </div>
    ...
    <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-all flex flex-col h-full">
      <div className="flex items-start justify-between mb-4 gap-4">
        <div className="min-w-0 flex-1">
          <div className="mb-1">
            <span className="text-[10px] font-bold px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded-lg uppercase tracking-wider truncate max-w-[120px] inline-block">{resource.course.split(':')[0]}</span>
          </div>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-slate-100 mt-auto gap-2">
        <div className="flex flex-col min-w-0 flex-1">
          ...
          <span className="text-xs font-bold text-slate-700 truncate">{resource.userName}</span>
        </div>
        <a 
          href={resource.link} 
          target="_blank" 
          rel="noopener noreferrer" 
          className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-3 sm:px-4 py-2 rounded-xl text-xs font-bold shadow-lg shadow-indigo-100 transition-all active:scale-95 shrink-0"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
          <span className="hidden sm:inline">Download</span> ({resource.downloads})
        </a>
      </div>
    </div>
  );
