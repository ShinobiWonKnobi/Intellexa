...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Persistence effects
  useEffect(() => {
    if (user) localStorage.setItem('intellexa_user', JSON.stringify(user));
  }, [user]);
  ...
  const addQuestion = (q: Partial<Question>) => {
    ...
    setUser(prev => prev ? { ...prev, contributions: prev.contributions + 1 } : null);
  };
  ...
  const markBestAnswer = (questionId: string, answerId: string) => {
    ...
    // Award karma to the answerer if it's the current user
